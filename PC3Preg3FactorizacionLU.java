//import java.io.*;         public static Result LuBloques(double [][]M) se hace uso de BLOCKINGQUEUE
//import javax.swing.*;
...
public class PC3Preg3FactorizacionLU{    //N tiene que cumplir          N>=4
	private static String FILENAME = "DATAPC3Preg3FactorizacionLU.TXT"; 	//se crea la DATA y se guarda en FILENAME
    ...
    //-----------------------------------------------------------------------------------------------------------------
    private static double[][] Submatriz(double[][] M, int filaIni, int colIni, int tam) {
        double[][] submatriz = new double[tam][tam];
        for (int i = 0; i < tam; i++) {
            for (int j = 0; j < tam; j++) {
                submatriz[i][j] = M[filaIni + i][colIni + j];
            }
        }
        return submatriz;
    }
	...
	public static Result LuBloques(double [][]M){
         ...
         // aqui se intentara usar una cola para sincronizar el trabajo de los 4 hilos(MODIFICACION)
	     BlockingQueue<double[][]> COLA = new LinkedBlockingQueue<>();
         final Matrices m = new Matrices();
         int g=N/2;
		 try{
            // Dividir la matriz y encolar los cuadrantes
            COLA.put(Submatriz(M,0, 0, g)); // A11
            COLA.put(Submatriz(M, 0, g, g));        // A12
            COLA.put(Submatriz(M, g, 0, g));         // A21
            COLA.put(Submatriz(M, g, g, g));                // A22
        }
